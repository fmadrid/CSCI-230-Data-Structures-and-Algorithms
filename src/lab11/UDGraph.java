...
/**
 * 
 *  The UDGraph class represents an unweighted directed graph. Implemented with an adjacency matrix.
 */
...
public class UDGraph {
   ...
   */
  public boolean validVertex(int v) { return (v >= 0) && (v < vertices); }
  ...
  public boolean hasEdge(int origin, int destination) {
	  if (validVertex(origin) && validVertex(destination)) return adjMatrix[origin][destination];
	  else return false;
  }

  /**
   *  Creates the edge (origin, destination).  If the edge did not already exists, increments the edge count.
   *  @param origin the origin vertex.
   *  @param edstination the destination vertex.
   */
  public void addEdge(int origin, int destination) {
	  if (validVertex(origin) && validVertex(destination) && !adjMatrix[origin][destination]) {
  ...
  /**
   *  Deletes the edge (origin, destination).  If the edge existed, decrements the edge count.
   *  @param origin the origin vertex.
   *  @param destination the destination vertex.
   */
   ...
   *    The new graph has an edge (v, w) if and only if there is a path of length 2 from v to w in "this" graph.
  ...
  /**
   *  Returns a new UDGraph with the same vertices as "this" UDGraph.
   *    The new graph has an edge (v, w) if and only if there is a path of length "length" from v to w in "this" graph.
  ...
  /**
   *  Returns a String representing the adjacency matrix, including the number of vertices and edges.
